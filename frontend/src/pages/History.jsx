            ...
            {[...plans].sort((a, b) => new Date(b.date) - new Date(a.date)).map((plan) => (
              ...
              >
                <CardHeader>
                  ...
                  <CaretDown size={14} style={{ marginLeft: "auto", transition: "transform 0.2s", transform: openId === plan._id ? "rotate(180deg)" : "rotate(0deg)" }} aria-hidden="true" />
                </CardHeader>
                {openId === plan._id && (
                  <>
                    <EnergyRow>
                      <EnergyNumbers>
                        ...
                        <Arrow aria-label={plan.currentEnergy >= plan.startingEnergy ? "energi ökade" : "energi minskade"}
                          $energy={plan.currentEnergy}
                        >
                    ...
                    <ActivityChips>
                      {[...plan.activities]
                        ...
                        .map(a => <Chip key={a._id} $positive={a.energyImpact > 0}>{a.name}</Chip>)}
                    </ActivityChips>
                    <NoteArea
